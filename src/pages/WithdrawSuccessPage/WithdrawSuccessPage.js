...
const WithdrawSuccessPage = () => {
  ...
  const NavigateDashboard = () => {
    navigate(`/users/seller/dashboard/${userId}`);
  };
  const NavigateSellerSearch = () => {
    navigate("/users/seller_search");
  };
    ...
    <div className="sm:p-12 p-2">
      ...
      <div className="py-12 space-y-6 w-11/12 sm:w-1/2 mx-auto">
        ...
        <div className="pt-6 sm:w-[400px] mx-auto space-y-4 sm:space-y-6">
          <h3 className="font-bold text-xl text-center text-[#121F3E] lg:text-2xl">
            Withdraw Complete!
          </h3>
          <p className="text-[#8E8E8E] text-center">
            You will be notified further for this job
            <span className="text-[#121F3E] ml-2">Thankyou!</span>{" "}
          </p>

          <div className="space-y-3 pt-6">
            <button
              onClick={NavigateSellerSearch}
              className="bg-[#00538F] text-white font-bold rounded-md px-4 py-2 lg:px-6 lg:py-2 w-full mx-auto"
            >
              Browse more jobs
            </button>
            <button
              onClick={NavigateDashboard}
              className="border-2 border-[#FF3B30] text-[#FF3B30] font-bold rounded-md px-4 py-2 lg:px-6 lg:py-2 w-full mx-auto"
            >
              Back to dashboard
            </button>
          </div>
